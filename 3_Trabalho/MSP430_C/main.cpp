...
// ABRIR O RELÊ
 void Abrir_Porta(){
     while(1){
     digitalWriteA(1, LED1, HIGH); //Ativar o rele
     break;
     }
 }
...
// FECHAR O RELÊ
 void Fechar_Porta(){
     while(1){
     digitalWriteA(1, LED1, LOW); //Desativar o rele
     break;
     }
    }
...
void apagador_reg() {
 ...
 while (x) {
      if(Serial_UART.available() != 0) {
        ...
        Serial_UART.print("DIGITE O NUMERO DO REGISTRADO QUE VOCÊ DESEJA APAGAR !\n");
        delayms(2000);
        }
  }
}
...
int setar_reg(volatile int p) {
  int y = 1;
  int inByteS = 0;
 while (y) {
      if(Serial_UART.available() != 0) {
          Serial_UART.readInt();
          Serial_UART.print(inByteS);
                 if (inByteS != 0 ) { //
                  k = (inByteS - 48);
                  p = l;
                  Serial_UART.print("POSIÇÃO ATUAL: \n");
                  Serial_UART.print(p);
                  y = 0;
        ...
        Serial_UART.print("DIGITE A POSIÇÃO QUE VOCE QUER COLOCAR O REGISTRADOR !\n");
        delayms(2000);
        }
  }
 return (p);
           ...
           Serial_UART.print(a);
              if(a == 49){ //CHAMA MODO DE COMANDO NO PC
               ...
               { //CHAMA MODO DE CADASTRO
                 ...
                 {
                 delayms(2000);
                 Serial_UART.print("\nPASSE O CARTAO QUE VAI SER CADASTRADO\n");
