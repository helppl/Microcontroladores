...
Rc522::Rc522(int chipSelectPin, int NRSTPD)
{
	pinModeA(2, chipSelectPin, OUTPUT);
	_chipSelectPin = chipSelectPin;
	pinModeA(1, NRSTPD, OUTPUT);
	digitalWriteA(1, NRSTPD, HIGH);
	_NRSTPD = NRSTPD;
}
...
* Return: Null
*/
void Rc522::WriteReg(unsigned char addr, unsigned char val)
{
	digitalWriteA(2,_chipSelectPin, LOW);

	Serial_SPI.transfer((addr << 1) & 0x7E);
	Serial_SPI.transfer(val);

	digitalWriteA(2,_chipSelectPin, HIGH);
}
...
*/
unsigned char Rc522::ReadReg(unsigned char addr)
{
	unsigned char val;
	digitalWriteA(2,_chipSelectPin, LOW);

	Serial_SPI.transfer(((addr << 1) & 0x7E) | 0x80);
	val = Serial_SPI.transfer(0x00);
...
* Return: null
*/
void Rc522::Init(void)
{
	digitalWriteA(1,_NRSTPD, HIGH);
...
*/
unsigned char Rc522::Request(unsigned char reqMode, unsigned char *TagType)
{
...
* Return: return MI_OK if successed
*/
unsigned char Rc522::ToCard(unsigned char command, unsigned char *sendData, unsigned char sendLen, unsigned char *backData, unsigned int *backLen)
{
...
* Return: return MI_OK if successed
*/
unsigned char Rc522::Anticoll(unsigned char *serNum)
{
	unsigned char status;
	unsigned char i;
...
* Return: Null
*/
void Rc522::CalulateCRC(unsigned char *pIndata, unsigned char len, unsigned char *pOutData)
{
	unsigned char i, n;
...
*/
unsigned char Rc522::SelectTag(unsigned char *serNum)
{
	unsigned char i;
	unsigned char status;
	unsigned char size;
...
* Return: return MI_OK if successed
*/
unsigned char Rc522::Auth(unsigned char authMode, unsigned char BlockAddr, unsigned char *Sectorkey, unsigned char *serNum)
{
	unsigned char status;
	unsigned int recvBits;
	unsigned char i;
...
* Return: return MI_OK if successed
*/
unsigned char Rc522::ReadBlock(unsigned char blockAddr, unsigned char *recvData)
{
	unsigned char status;
	unsigned int unLen;
...
* Return: return MI_OK if successed
*/
unsigned char Rc522::WriteBlock(unsigned char blockAddr, unsigned char *writeData)
{
	unsigned char status;
	unsigned int recvBits;
	unsigned char i;
