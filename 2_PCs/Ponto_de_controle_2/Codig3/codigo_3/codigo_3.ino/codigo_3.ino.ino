...
void setup()
{
  ...
  pinMode(rele, OUTPUT);
  pinMode(PUSH2, INPUT);                    // Botão para abrir por dentro a porta  
...
void loop()
{
  ...
  if (status == MI_OK)
  {
    ...
    if ((serNum[0] == 0xBB && serNum[1] == 0x01 && serNum[2] == 0xB5 && serNum[3] == 0xA9 && serNum[4] == 0xA6) || PUSH2 == HIGH)
    {
      Serial.println("Acesso do bilhete unico autorizado.\n");
      //digitalWrite(rele, HIGH); // ativa rele, abre a trava solenoide
